...
- (void)setRotationSwitch:(bool)rotationSwitch;
...
/* switch aduio transport type */
- (BOOL)switch2MediaTransport:(BOOL)isMedia;

#ifdef USED_ENHANCE
+ (BOOL)initializeEnhance:(NSString *)filePath ownerKey:(NSString *)ownerKey pin:(NSString *)pin;

+ (BOOL)unInitializeEnhance;

+ (BOOL)isInitializeEnhanced;

+ (void)registerObserver:(RTCQsapObserver)observer;
